  ...
  return (
    <div className={`app-shell ${controller.feedback ? `feedback-${controller.feedback}` : ''}`}>
      ...
      <main className="main-content">
        {view === 'board' && <BoardView state={data} stats={stats} search={controller.search} selectedSceneId={controller.selectedSceneId} setSelectedSceneId={controller.setSelectedSceneId} focusId={controller.boardFocusId} setFocusId={controller.setBoardFocusId} onSelectCard={controller.setSelectedCardId} onStartRun={(config) => { controller.setRunConfig(config); controller.startRun(config) }} onOpenRun={() => setView('run')} />}
        {view === 'run' && <RunView session={controller.runSession} state={data} config={controller.runConfig} setConfig={controller.setRunConfig} onStart={controller.startRun} onReveal={controller.revealRunCard} onAnswer={controller.answerRun} onTypedChange={controller.setTypedAnswer} onOpenCard={controller.setSelectedCardId} onOpenBoard={() => setView('board')} />}
