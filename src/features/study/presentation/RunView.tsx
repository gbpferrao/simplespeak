...
export function RunView({ session, state, config, setConfig, onStart, onReveal, onAnswer, onTypedChange, onOpenCard, onOpenBoard }: RunViewProps) {
  ...
  return (
    <section className="view run-view focused-run-page">
      ...
      <div className="run-meta-row"><span><span className="live-dot" /> {t('run.cardPositionShort', { current: session.currentIndex + 1, total: session.cards.length })}</span><span>{session.hits} {t('history.hits')} - {session.misses} {t('run.missed').toLowerCase()} - {session.reveals} {t('run.reveal').toLowerCase()}</span><span className={`stability-inline status-${learning.status}`}>{t('run.stabilityNow', { status: statusLabel(learning.status, locale), count: Math.round(retrievability(learning) * 100) })}</span></div>
      <div className={`study-stage ${session.revealed ? 'is-revealed' : ''}`}>
        <div className="study-stage-top"><span className="stage-scene"><span className="scene-tab-dot" style={{ background: sceneFor(starterPack.scenes, card.sceneId)?.accent }} />{sceneFor(starterPack.scenes, card.sceneId)?.name}</span><button type="button" onClick={() => onOpenCard(card.id)}><PanelRight size={14} /> {t('run.details')}</button></div>
        <div className="study-card-wrap"><button className={`study-card ${image ? 'has-image' : 'word-card'} ${session.revealed ? 'revealed' : ''}`} type="button" onClick={session.revealed ? undefined : onReveal}>{!session.revealed && image && <img src={image} alt="" />}{!session.revealed && !image && <span className="study-word-fallback">{card.target}</span>}{session.revealed && <div className="study-card-back"><span className="back-label">{t('run.targetWord')}</span><strong>{card.target}</strong><span className="back-translation">{card.origin}</span><span className="back-example">&quot;{card.example?.target ?? card.target}&quot;</span><span className="back-note">{state.notes[card.id] || card.note || t('card.noMnemonic')}</span></div>}{!session.revealed && <span className="study-card-hint">{t('run.tapReveal')}</span>}</button></div>
        <div className="study-prompt">{session.revealed ? <><Eye size={15} /> {t('run.backOpen')}</> : image ? <><Keyboard size={15} /> {t('run.recall')}</> : <><EyeOff size={15} /> {t('run.noImage')}</>}</div>
        {session.revealed ? <div className="answer-actions revealed-actions"><button className="primary-button" type="button" onClick={() => onAnswer('reveal', true)}><RotateCcw size={15} /> {t('run.continueMiss')}</button></div> : <form className="answer-form" onSubmit={handleSubmit}><div className="answer-input-wrap"><Keyboard size={16} /><input autoFocus value={session.typedAnswer} onChange={(event) => onTypedChange(event.target.value)} placeholder={t('run.typeTarget')} autoComplete="off" /><span>{t('run.enter')}</span></div><div className="answer-actions"><button className="hit-button" type="button" onClick={() => onAnswer('hit', false)}><Check size={16} /> {t('run.iKnewIt')}</button><button className="miss-button" type="button" onClick={() => onAnswer('miss', false)}><X size={16} /> {t('run.missed')}</button><button className="reveal-button" type="button" onClick={onReveal}><Eye size={16} /> {t('run.reveal')}</button></div></form>}
      </div>
...
function RunSetup({ locale, state, config, setConfig, onStart, onOpenBoard }: { locale: SupportedLocale; state: PersistedState; config: RunConfig; setConfig: (config: RunConfig) => void; onStart: () => void; onOpenBoard: () => void }) {
  ...
  const presets: Array<{ id: RunConfig['preset']; title: string; description: string; icon: typeof Timer }> = [
    ...
    { id: 'all', title: t('run.allWords'), description: t('settings.loadedPack'), icon: Layers3 },
  ...
  return (
    <section className="view run-setup-view">
      <div className="run-header"><div><div className="eyebrow"><Target size={14} /> {t('run.chooseReturn')}</div><h1>{t('run.chooseReturn')}</h1><p>{t('settings.rhythmLead')}</p></div><button className="soft-button" type="button" onClick={onOpenBoard}><Map size={15} /> {t('nav.backToBoard')}</button></div>
      <div className="setup-grid">
        ...
        <aside className="setup-aside"><div className="setup-preview-card"><span className="eyebrow"><Sparkles size={13} /> {t('settings.returnWhen')}</span><strong>{t('run.dueNow', { count: stats.due })}</strong><p>{t('settings.engine')}</p><div className="preview-stat"><span>{t('settings.dailyTarget')}</span><strong>{t('run.cards', { count: state.settings.dailyTarget })}</strong></div><div className="preview-stat"><span>{t('settings.overallHorizon')}</span><strong>{t('stability.days', { count: state.settings.timeframeDays })}</strong></div></div><div className="setup-note"><ShieldCheck size={16} /><span>{t('run.continueMiss')}</span></div></aside>
      </div>
    </section>
  )
}
