...
export function RunView({ session, state, config, setConfig, onStart, onReveal, onAnswer, onTypedChange, onOpenCard, onOpenBoard }: RunViewProps) {
  ...
  return (
    <section className="view run-view focused-run-page">
      <div className="run-header"><div><div className="eyebrow"><Play size={14} /> Active run - {session.label}</div><h1>One card at a time.</h1><p>Try the target first. Reveal is a miss by default; a typed answer can be a hit.</p></div><button className="soft-button" type="button" onClick={onOpenBoard}><Map size={15} /> Back to atlas</button></div>
