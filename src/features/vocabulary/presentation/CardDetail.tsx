  ...
  return (
    <div className="overlay-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <aside className="modal-card detail-modal" role="dialog" aria-modal="true" aria-labelledby="card-detail-title">
        <div className="drawer-header"><div><span className="eyebrow"><BookOpen size={13} /> Word card - {humanize(card.sense ?? 'primary')}</span><h2 id="card-detail-title">{card.target}</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close card details"><X size={18} /></button></div>
        <div className="detail-scroll">
          ...
          <section className="detail-section"><div className="section-label"><span><Lightbulb size={14} /> Mnemonic note</span><span>private to this card</span></div><textarea value={note} onChange={(event) => setNote(event.target.value)} rows={4} placeholder={card.note ?? 'Write a private mnemonic for this meaning...'} /><button className="text-button" type="button" onClick={() => onSaveNote(note)}><Save size={13} /> Save note</button></section>
          <section className="detail-section example-section"><div className="section-label"><span><BookOpen size={14} /> Context</span><span>pack content</span></div><p className="example-target">&quot;{card.example?.target ?? card.target}&quot;</p><p className="example-origin">{card.example?.origin ?? card.origin}</p></section>
